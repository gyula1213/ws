...
public class Colorflip {
	...
	private int[][] startTable = null; // Kiinduló tábla
	private int cnt = 0; // tippek száma
	private int[][] actTable = null; // Actuális tábla
	private int[][] actSolver = null; // Ahol true van, azokat a mezõket kell megbökni a megoldáshoz
...
//		}
	...
	private void generate() {
		if (size % 2 == 0) {
			...
			actSolver = new int[size][size]; // alaphelyzetben hozzuk létre
	...
	private int[][] step(int[][] tomb, int x, int y) {
		...
		for (int i = 0; i < tomb.length; i++) {
			for (int j = 0; j < tomb[i].length; j++) {
				if (x == (i + 1) || y == (j + 1))
					ret[i][j] = 1 - tomb[i][j]; // megfordítjuk
				else
					ret[i][j] = tomb[i][j];
	...
	/**
	 * Visszatérés true, ha mind egyforma, false, ha nem
	 * 
	 * @param tomb
	 * @return
	 */
	...
	/**
	 * Egy mezo megforditasa ugy, hogy rábökünk az összes soraban, vagy oszlopában
	 * állo mezore A bokeseket megjegyezzuk
	 */
	...
	/**
	 * megnezi, hogy a forditottja nem rovidebb-e ha igen, akkor megfordítja a
	 * bokesi tombot
	 */
	...
	/**
	 * lemasolja solverTabla tartalmat ha igen, akkor megfordítja a bokesi tombot
	 */
